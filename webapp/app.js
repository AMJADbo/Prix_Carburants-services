...
function renderStations() {
  ...
  // Sauvegarde le nombre de résultats et met à jour le bandeau
...
function renderStationsOnMap(stations) {
  ...
  // Ajoute une légende fixe
  let legend = document.getElementById('map-legend');

  if (!legend) {
    legend = document.createElement('div');
    legend.id = 'map-legend';
    legend.className = 'map-legend'; // La classe CSS fera tout le travail

    legend.innerHTML = `
      <div class="legend-item" id="legend-user">
        ...
        <span id="legend-user-label">Votre position</span>
      </div>
      <div class="legend-item" id="legend-station">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#00c950" class="bi bi-circle-fill" viewBox="0 0 16 16">
          <circle cx="8" cy="8" r="8"/>
        </svg>
        <span id="legend-station-label">Stations essence</span>
      </div>
    `;
